...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Quaternion<float> nq = q.IntegrateBodyRate(gyro, dtIMU);

  float predictedPitch = nq.Pitch();
  float predictedRoll = nq.Roll();
  ekfState(6) = nq.Yaw();
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  // MAGNETOMETER UPDATE
  // Hints: 
  //  - Your current estimated yaw can be found in the state vector:
  //  - Make sure to normalize the difference between your measured and estimated yaw
  //    (you don't want to update your yaw the long way around the circle)
  //  - The magnetomer measurement covariance is available in member variable R_Mag
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////

  z(0) = CONSTRAIN(magYaw, -M_PI, M_PI);
  hPrime(0,6) = 1.0f;
  zFromX(0) = CONSTRAIN(ekfState(6), -M_PI, M_PI);
